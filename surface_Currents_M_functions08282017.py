...
class get_fvcom():
    ...
    def current_track_new(self,track_way): #,point,leh):
        ...
        for j in range(numep):
            #print epoints[i][0],epoints[i][1] # (lon,lat)
            getk = self.get_track(epoints[j][0],epoints[j][1],track_way)
            #if not tdex: continue
            cts.append(getk)#; dexs.append(tdex[0])
            print j,len(getk)
        return cts


    def get_track(self,lon,lat,track_way): #,b_index,nvdepth,,bcon 
        '''
        ...
        #while True:
            ...
            if track_way=='backward':
                u_t1 = -self.u[:,elementindex][0]; v_t1 = -self.v[:,elementindex][0]
            ...
            u = np.mean(u_t1) ; v = np.mean(v_t1); 
            sd = math.sqrt(u**2+v**2) # m/s
            # Condition 1
            if sd < 0.1:
                print 'u,v',u,v
                return fpoints
