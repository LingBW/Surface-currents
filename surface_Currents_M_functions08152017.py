...
class get_fvcom():
    ...
    def get_data(self,point,leh):
        ...
        (lat,lon) = point
        self.lonl,self.latl,self.points,self.ndex = self.shrink_data(lon,lat,self.lonc,self.latc,leh)
        #self.ndex [44257, 44258, 44259,,,]
        #print 'self.ndex',self.ndex
    ...
    def current_track_new(self,track_way): #,point,leh):
        cts = {}; dexs = []      
        #self.lonk,self.latk = self.shrink_data(lon,lat,self.lons,self.lats,leh)
        epoints = np.vstack((self.lonl,self.latl)).T #[::3]
        cts[self.mTime[0]] = []
        numep = len(epoints)
        for i in range(numep): #self.ndex
            ct = {}
            getk,tdex = self.get_track(epoints[i][0],epoints[i][1],0,track_way)
            if not tdex: continue
            ct[str(tdex[0][0])] = [getk]
            cts[self.mTime[0]].append(ct); dexs.append(tdex[0])
        #print self.ndex,dexs # Same index
        dexst = dexs
        t = abs(self.hours)    
        for i in xrange(t):
            if i ==0: continue
            #pdex = [a for a in dexs if a not in dexst]
            pdex = []
            for a in dexs:
                if a not in dexst:
                    pdex.append(a)
            print i,len(pdex),len(dexs),len(dexst)#,pdex
            cts[self.mTime[i]] = []; dexst = []
            
            #cl = []; sd = []; dt = []            
            #for j in range(len(cts[self.mTime[i-1]])):
            for j in cts[self.mTime[i-1]]:
                ct = {}; ct[j.keys()[0]]=[]
                for l in j.values()[0]:
                    
                    getk,tdex = self.get_track(l[-1][0],l[-1][1],i,track_way) # forecast one point (lon,lat)
                    #print type(getk['lon']),type(getk['lat']),type(getk['layer']),type(getk['spd'])
                    if not tdex: continue
                    ct[j.keys()[0]].append(getk); dexst.append(tdex[0])
                # 
                if len(ct.values()[0])==0 or int(j.keys()[0]) in pdex:
                    getk,tdex = self.get_track(self.lonc[[int(j.keys()[0])]][0],self.latc[[int(j.keys()[0])]][0],i,track_way)
                    if not tdex: continue
                    ct[j.keys()[0]].append(getk); dexst.append(tdex[0])
                    if len(ct.values()[0])>3:
                        del ct[j.keys()[0]][:-3]; print 'Del'
                    
                cts[self.mTime[i]].append(ct)
                
                '''pspeed = math.sqrt((getk[1][0]-getk[0][0])**2+(getk[1][1]-getk[0][1])**2)       
                if len(cl)<300:
                    cl.append(getk); sd.append(pspeed);dt.append(tdex[0])
                else:
                    ix = np.argmin(sd)
                    if pspeed > sd[ix]:
                        cl[ix] = getk; sd[ix] = pspeed; dt[ix] = tdex[0]#'''
            #currents_points[k] = cl; print len(cl)                
            #cts[self.mTime[i]].extend(cl); dexst.extend(dt)
        return cts


    def get_track(self,lon,lat,ti,track_way): #,b_index,nvdepth,,bcon 
        '''
        Get forecast point start at lon,lat
        '''
        #modpts = dict(lon=[lon], lat=[lat], time=[], spd=[]) #model forecast points, layer=[]
        fpoints = [(lon,lat)]; tdexs = []
        for k in range(1):
            try:
                if self.modelname == "GOM3" or self.modelname == "30yr":
                    lonp,latp = self.nearest_point(lon, lat, self.lonl, self.latl,0.2)
                    #lonn,latn = self.nearest_poi:nt(lon,lat,self.lonk,self.latk,0.3)
                ...
                tdexs.append(elementindex)
                '''if k == 0:
                    dex = elementindex #'''
            except:
                #print 'None'
                return fpoints,tdexs#,0  
            ...
            if track_way=='backward':
                u_t1 = -self.u[t-1-ti,elementindex][0]; v_t1 = -self.v[t-1-ti,elementindex][0]
                #modpts['time'].append(self.mTime[t-1-i])
            else:
                u_t1 = self.u[ti,elementindex][0]; v_t1 = self.v[ti,elementindex][0]
            ...
            fpoint = (temlon,temlat); fpoints.append(fpoint)
            if self.p.contains_point(fpoint):                
                lon = temlon; lat = temlat
            else:
                return fpoints,tdexs
        return fpoints,tdexs
    ...
    def nearest_point(self, lon, lat, lons, lats, length):  #0.3/5==0.06
        '''Find the nearest point to (lon,lat) from (lons,lats),
           ...
           author: Bingwei'''
        self.p = Path.circle((lon,lat),radius=length)
        ...
        for i in xrange(len(points)):
            if self.p.contains_point(points[i]):# .contains_point return 0 or 1
    ...
    def shrink_data(self,lon,lat,lons,lats,le):
        lont = []; latt = []; ndex = [] # ndex: the index of nodes
