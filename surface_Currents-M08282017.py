...
from surface_Currents_M_functions08282017 import get_fvcom,draw_basemap,get_drifter
...
track_hours = 12    #MODEL track time(days)
...
spds = []
for j in currents_points:
    #kl = currents_points[k]#; cl = []; sd = []
    for t in range(len(j)-1):
        #for t in range(len(j)-1):
        dx = j[t+1][0]-j[t][0]; dy = j[t+1][1]-j[t][1]
...
norm = mpl.colors.Normalize(vmin=0, vmax=max(spds))#
...
def animate(m): #del ax.collections[:]; del ax.lines[:]; ax.cla(); ax.lines.remove(line)        
    ...
    for j in currents_points:
        for k in range(int(len(j)/10)):
            if m-k*10 >=0:
                n=m-k*10
                if n<5:    
                    if len(j)>n:
                        ps = j[:n+1]
                        mp = math.sqrt((ps[-1][0]-ps[0][0])**2+(ps[-1][1]-ps[0][1])**2)/len(ps)
                        cl = colors[np.argmin(abs(crang-mp))]
                        jp = np.vstack(ps).T
                        ax.plot(jp[0],jp[1],color=cl)
                    #else:
                else:
                    if len(j)>n:
                        ps = j[n-4:n+1]
                        ...
                        if ix<5:
                            ...
                            cl = colors[np.argmin(abs(crang-mp))]
                            jp = np.vstack(ps).T
                            #jp = np.vstack(j[ix-4:ix+1]).T
                            ax.plot(jp[0],jp[1],color=cl)
           
    
            #ax.arrow(j[t][0],j[t][1],dx,dy,fc=cl,ec=cl,head_width=0.005, head_length=0.01)#'''
            
    # plot the scale
    #ax.plot([lmaxlon0,lmaxlon1],[lminlat0,lminlat1],'-|',color='k',linewidth=3)
    #ax.text(lmaxlon1,lminlat1+0.004,'1 mph')#'''
anim = animation.FuncAnimation(fig, animate, frames=101,interval=500) #,
