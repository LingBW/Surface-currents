        ...
        for j in xrange(len(kl)):
            if any(kl[j]['time']==toltime[n]):
                ...
                if nu < len(kl[j]['lon'])-1 :    # if Index less than length of list, plot index and index+1 
                    ...
                    '''dx=kl[j]['lon'][nu+1]-kl[j]['lon'][nu]; dy=kl[j]['lat'][nu+1]-kl[j]['lat'][nu]; m = np.hypot(dx,dy)
                    Q=ax.quiver(kl[j]['lon'][nu],kl[j]['lat'][nu],dx,dy,m,scale_units='xy', angles='xy', scale=1)#'''
                    #plt.quiverkey(Q, 0.9, 0.9, 1, r'$1 \frac{mile}{h}$', labelpos='E',coordinates='figure')
    # plot the scale
